...
# defining the bcc system, i.e., with 2 times n**3 atoms
def bccSystem(n, N, A, sigma):
	# n = times A for super cell
	# N = number of atoms
	x_mesh, y_mesh, z_mesh = np.meshgrid(np.arange(0, n * A, A), np.arange(0, n * A, A), np.arange(0, n * A, A))
	x_mesh = x_mesh.reshape(-1)
	...
	for i in range(2):
		...
		# bcc observe a unit cell with 2 atoms, with orthogonal lattice vectors
		# atom 1: 0.0, 0.0, 0.0
		# atom 2: 0.5, 0.5, 0.5
		# above coordinates in fractions
		rvec[st:en, 0] = x_mesh + a * A * np.ones(x_mesh.shape)
...
# defining the simple cubic system, i.e., with 2 times n**3 atoms
def scSystem(n, N, A, sigma):
	# n = times A for super cell
	# N = number of atoms
	x_mesh, y_mesh, z_mesh = np.meshgrid(np.arange(0, n * A, A), np.arange(0, n * A, A), np.arange(0, n * A, A))
	x_mesh = x_mesh.reshape(-1)
	...
	for i in range(1):
		...
		# simple cubic observe a unit cell with 1 atoms, with orthogonal lattice vectors
		# atom 1: 0.0, 0.0, 0.0
		# above coordinates in fractions
		rvec[st:en, 0] = x_mesh + a * A * np.ones(x_mesh.shape)
		rvec[st:en, 1] = y_mesh + b * A * np.ones(y_mesh.shape)
...
print('\n Default position generation: FCC')
temp = input('  Do you want to change it? [bcc/sc/no]  ')
if temp in ['bcc', 'BCC', 'Bcc']:
	print('  Chosen system: BCC')
	N = 2 * n_lattice**3
	rT0_rv = bccSystem(n_lattice, N, A, sigma)
elif temp in ['sc', 'SC', 'Sc']:
	print('  Chosen system: SC / Simple Cubic')
	N = n_lattice**3
	rT0_rv = scSystem(n_lattice, N, A, sigma)
elif temp in ['no', 'No', 'NO', 'N', 'n']:
	print('  Continuing with FCC system')
